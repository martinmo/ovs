...
{
    ovs_assert(byteq_is_empty(q));
    /* Do not simply reset to zero, but to the next multiple of size because we currently
     * have callers that depend on increasing values. */
    unsigned int pos = q->head & (q->size - 1);
    if (pos) {
        /* only advance head if we are not already at a multiple */
        q->head += q->size - pos;
    }
    q->tail = q->head;
}
